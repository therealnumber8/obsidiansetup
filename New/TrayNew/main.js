    ...
    const obsidianIcon = nativeImage.createFromDataURL(
      ...
      contextMenu = Menu.buildFromTemplate([
        ...
        },
        {
          type: "normal",
          label: ACTION_HIDE,
          accelerator: plugin.settings.toggleWindowFocusHotkey2 || plugin.settings.toggleWindowFocusHotkey,
          click: hideWindows,
        },
        { type: "separator" },
...
const registerHotkeys = () => {
    ...
    try {
      const { toggleWindowFocusHotkey, toggleWindowFocusHotkey2, quickNoteHotkey } = plugin.settings;
      if (toggleWindowFocusHotkey) {
        globalShortcut.register(toggleWindowFocusHotkey, toggleWindows);
      }
      if (toggleWindowFocusHotkey2) {
        globalShortcut.register(toggleWindowFocusHotkey2, toggleWindows);
      }
      if (quickNoteHotkey) {
  ...
  unregisterHotkeys = () => {
    ...
    try {
      ...
      globalShortcut.unregister(plugin.settings.toggleWindowFocusHotkey2);
...
const OPTIONS = [
  ...
  {
    key: "toggleWindowFocusHotkey2",
    desc: "Second global hotkey for toggling window focus. " + ACCELERATOR_FORMAT,
    type: "hotkey",
    default: "",
    onBeforeChange: unregisterHotkeys,
    onChange: registerHotkeys,
